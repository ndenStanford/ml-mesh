...
def main() -> None:
    ...
    for idx in range(len(sum_settings)):
        summaries = pipelines[idx](model_card.model_inputs.sample_documents[idx],
                       min_length=32,
                       max_length=128,
                       num_beams=1)
    ...
    # model artifact
    hf_pipeline_local_dir_en = os.path.join(model_card.local_output_dir, "hf_pipeline_en")
    hf_pipeline_local_dir_frde = os.path.join(model_card.local_output_dir, "hf_pipeline_frde")
    hf_pipeline_local_dir_es = os.path.join(model_card.local_output_dir, "hf_pipeline_es")
    hf_pipeline_local_dir_ca = os.path.join(model_card.local_output_dir, "hf_pipeline_ca")
    hf_pipeline_local_dir_it = os.path.join(model_card.local_output_dir, "hf_pipeline_it")
