...
def test_integration_summarization_model(summarization_model, payload):
    """Integration test for SummarizationServedModel."""
    response = summarization_model.predict(PredictRequestSchema(**payload))

    assert response.data.attributes.summary is not None
    # assert response.data.attributes.summary == expected_response
    # desired_length = payload["data"]["parameters"]["desired_length"]
    # assert len(response.data.attributes.summary) <= desired_length
...
def test_invalid_language(summarization_model, payload):
    """Test for invalid language."""
    response = summarization_model.predict(PredictRequestSchema(**payload))

    assert len(response.data.attributes.summary) == 0
