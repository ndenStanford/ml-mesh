...
@pytest.mark.parametrize(
    ...
    [
        ('prajjwal1/bert-tiny',AutoModelForSequenceClassification,'text-classification-model'),
        ('prajjwal1/bert-tiny',AutoModel,'feature-extraction-model')
    ]
)
def test_create_and_save_neuron_compiled_model(huggingface_model_reference, huggingface_model_type, test_output_dir, export_handle):

    huggingface_model = huggingface_model_type.from_pretrained(huggingface_model_reference)
    ...
    neuron_compiled_model.save_pretrained(
        os.path.join(
            test_output_dir,
            export_handle
        )
...
def test_load_and_score_neuron_compiled_model(export_handle, test_output_dir):
    ...
    model_inputs = {
        'input_ids':torch.ones((model_batch_size+1,model_max_length), dtype=torch.long),
        'attention_mask':torch.ones((model_batch_size+1,model_max_length), dtype=torch.long)
    }
...
def test_create_and_save_neuron_compiled_pipeline(huggingface_model_reference, huggingface_pipeline_task, test_output_dir):

    huggingface_pipeline = pipeline(task=huggingface_pipeline_task, model=huggingface_model_reference)
...
def test_load_and_score_neuron_compiled_pipeline(huggingface_pipeline_task, test_output_dir, torch_model_text_input):
