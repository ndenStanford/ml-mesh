...
def parse_args():
    ...
    if (
        args.framework == "tensorflow"  # noqa: W503
        and args.container_type == "inference"  # noqa: W503
        and args.framework_version == "2.12.0"  # noqa: W503
    ):
...
def query_bucket(instance_id, region):
    ...
    if instance_id is not None and region is not None:
        url = (
            ...
            "/dlc-containers-{1}.txt?x-instance-id={1}&x-framework={2}&x-framework_version={3}&x-py_version={4}&x-container_type={5}".format(  # noqa: E501
                region,
                instance_id,
                framework,
                framework_version,
...
def tag_instance(instance_id, region):
    ...
    tag = f"{framework}_{container_type}_{framework_version}_python{py_version}_{device}{cuda_version}_{os_version}"  # noqa: E501
