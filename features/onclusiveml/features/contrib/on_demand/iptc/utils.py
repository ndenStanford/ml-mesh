...
# Internal libraries
import logging
from onclusiveml.features.contrib.on_demand.iptc import (
...
def get_candidate_list(row: dict, level: int) -> list:
    """Get a candidate topic of a given node."""
    if level == 1:
        node_name = "root"
        candidate_list = list(CANDIDATE_DICT_FIRST.get(node_name, {"dummy": "dummy"}).values())
    elif level == 2:
        node_name = row["topic_1"]
        node_name = NAME_MAPPING_DICT_FIRST.get(node_name, node_name)
        candidate_list = list(CANDIDATE_DICT_SECOND.get(node_name, {"dummy": "dummy"}).values())
    elif level == 3:
        node_name = row["topic_2"]
        node_name = NAME_MAPPING_DICT_SECOND.get(node_name, node_name)
        candidate_list = list(CANDIDATE_DICT_THIRD.get(node_name, {"dummy": "dummy"}).values())
    elif level == 4:
        node_name = row["topic_3"]
        node_name = NAME_MAPPING_DICT_THIRD.get(node_name, node_name)
        candidate_list = list(CANDIDATE_DICT_FOURTH.get(node_name, {"dummy": "dummy"}).values())

    return candidate_list


def get_col_name(level: int) -> str:
    """Get a column name of each level."""
    col_name = f"topic_{level}_llm"
    return col_name


async def generate_label_llm(row: dict, session: Any, level: int) -> str:
        ...
        async with session.post(api_endpoint, headers=headers, json=payload) as response:
            ...
            if response.status == 200:
                ...
                try:
                    ...
                    logging.info(f"Failed to parse the response as JSON: {generated_content}")
                    return None
                ...
                logging.info(f"Failed to get a valid response from OpenAI API: {response.status}")
                response_text = await response.text()
                logging.info(f"Response text: {response_text}")
                return None

    except aiohttp.ClientError as e:
        logging.info(f"Network error occurred while calling OpenAI API: {e}")
        return None
...
async def enrich_dataframe(features_df: pd.DataFrame, level: int) -> pd.DataFrame:
    ...
    async with aiohttp.ClientSession() as session:
        tasks = [generate_label_llm(row, session, level) for _, row in features_df_copy.iterrows()]
