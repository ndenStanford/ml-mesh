...
from onclusiveml.ts.typing import Figsize
...
class TimeSeriesData:
    ...
    >>> import pandas as pd
    >>> df = pd.read_csv("/ts/data/air_passengers.csv")
    ...
    - categorical_var: A list of column names of categorical variables that are not
        required to be numerical. Default is None.
    - drop_duplicate_time: A bool variable to indicate whether to drop the duplicate
        time stamps.
    - cache_datetimes: A bool variable to indicate whether to use pandas cache to avoid
        recomputing datetime conversions
    ...
    def interpolate(
        ...
        **kwargs: Any,
        ...
        if method == "linear":
            df = df.resample(rule=freq).interpolate(method="linear")

        elif method == "ffill":
            df = df.resample(rule=freq).ffill()

        elif method == "bfill":
            df = df.resample(rule=freq).bfill()
        else:
            df = df.resample(rule=freq).interpolate(method=method, **kwargs)
