...
import logging
import warnings
from typing import Dict, Generator, Optional, Protocol, Union, cast
...
def pinball_loss(y_true: ArrayLike, y_pred: ArrayLike, threshold: float) -> float:
    ...
    For threshold t (0<t<1), true value y_true and forecast value y_pred,
    ...
    Attributes:
        ...
        threshold (float):  A float representing the threshold to be calculated.
    """
    y_true, y_pred = _arrays(y_true, y_pred)
    if threshold < 0:
...
def exceed(y_true: ArrayLike, y_pred: ArrayLike, threshold: float) -> float:
    ...
    Args:
        y_true (ArrayLike): the actual values.
        y_pred (ArrayLike): the predicted values.
        threshold (float): Thresholds to be calculated.
...
def coverage(y_true: ArrayLike, y_lower: ArrayLike, y_upper: ArrayLike) -> float:
    ...
    Args:
        y_true (ArrayLike): the actual values.
        y_lower (ArrayLike): the lower bound of the prediction interval.
        y_upper (ArrayLike): the upper bound of the prediction interval.
