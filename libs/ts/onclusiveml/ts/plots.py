...
def plot_scatter_with_confints(
    val: List[float], confint: ArrayLike, title: str
) -> Figure:
    """Plots a scatter plot with confidence intervals used to plot ACF and PACF.

    Args:
        val: (List[float]) list containing the values
        confint (ArrayLike): two dimensional ndarray; confint[:, 0] is the lower confidence
...
def plot_fitted_harmonics(
    ...
    Args:
        times (pd.Series): date list of the time series
        original_values (pd.Series): values of the original function
