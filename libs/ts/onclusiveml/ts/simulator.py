...
from onclusiveml.ts.typing import TimedeltaLike
...
class Simulator:
    ...
    def level_shift_sim(
        ...
        Args:
            ...
            cp_arr (Optional[List[int]]): Array of changepoint locations.
            level_arr (Optional[List[float]]): Array containing levels for each segment. Since the
            ...
            noise (float): std. dev of random Gaussian noise added.
            seasonal_period (int): periodicity of the time series.
            seasonal_magnitude (float): amplitude of the seasonality. Set this to 0, if
                you want a time series without seasonality.
            anomaly_arr (Optional[List[int]]): locations where we introduce an anomalous point.
            z_score_arr (Optional[List[float]]): same length as anomaly arr. This is the z-score of the
    ...
    def level_shift_multivariate_indep_sim(
        ...
        Args:
            cp_arr (Optional[List[int]]): Array of changepoint locations.
            level_arr (Optional[List[float]]): Array containing levels for each segment.
                Since the number of segments is one more than the number of
                changepoints, hence, the level arr should be longer than the
                cp_arr by one.
