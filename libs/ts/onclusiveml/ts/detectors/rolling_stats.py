...
# Internal libraries
from onclusiveml.ts.base.detector import DetectorModel
...
def calculate_mad(data_list: np.ndarray, **kwargs: Any) -> Union[float, np.ndarray]:
    """Calculate MAD: the mean (average) distance between each data value and the mean of the data set."""
    data_list_2dim = data_list.reshape((-1, data_list.shape[-1]))
...
def calculate_modified_z_scores_mad(
    ...
    Args:
        data_list (np.ndarray): input array
    """
    data_list_2dim = data_list.reshape((-1, data_list.shape[-1]))
...
class RollingStatsModel(DetectorModel):
    ...
    def _fit_predict_point_based(
        ...
        Args:
            data_interp (TimeSeriesData): The data on which to compute the
                stats.
            n_points_hist_data (int): How many points from the original data
                were historical data. Used to keep track of how many points
                need to be scored since historical_data and data are merged
                in fit_predict().
        """
        if self.extend_rolling_window:
    ...
    def fit_predict(
        ...
        **kwargs: Any,
        ...
        """Anomaly detection fit and prediction.

        Args:
            data (TimeSeriesData): time series to run anomaly detection on
