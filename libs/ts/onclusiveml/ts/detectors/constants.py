...
class PercentageChange:
    ...
    Attributes:
        ...
        skip_rescaling: bool, default value is False.
        For multi-variate TS, we need to rescale p-values so that alpha is still the threshold for rejection.
        ...
        min_perc_change: float, minimum percentage change, for a non zero score. Score will be clipped to zero
        if the absolute value of the percentage chenge is less than this value.
    """
    ...
    def _ttest_manual(self) -> Tuple[float, float]:
        """Scipy's t-test gives nan when one of the arrays has a size of 1.
    ...
    def _ttest_multivariate(self) -> None:
        ...
        # We are using a two-sided test here, so we take inverse_tcdf(self._p_value / 2) with
        # df = len(self.current) + len(self.previous) - 2
...
class AnomalyResponse:
    ...
    def _update_ts_slice(
        ...
        time = pd.concat([ts.time.iloc[1:], pd.Series(time, copy=False)])
        time.reset_index(drop=True, inplace=True)
        if self.num_series == 1:
            value = pd.concat([ts.value.iloc[1:], pd.Series(value, copy=False)])
    ...
    def extend(self, other: "AnomalyResponse", validate: bool = True) -> None:
        """Extends :class:`AnomalyResponse` with another :class:`AnomalyResponse` object.

        Args:
          other (AnomalyResponse): The other :class:`AnomalyResponse` object.
          validate (bool): A boolean representing if the contained
