...
from onclusiveml.ts.data.helpers import load_data
from onclusiveml.ts.timeseries import TimeSeriesData, TSIterator
...
EMPTY_TIME_SERIES = pd.Series([], name=DEFAULT_TIME_NAME, dtype=object)
EMPTY_VALUE_SERIES = pd.Series([], name=DEFAULT_VALUE_NAME, dtype=object)
EMPTY_VALUE_SERIES_NO_NAME = pd.Series([], dtype=object)
...
class TimeSeriesDataInitTest(TimeSeriesBaseTest):
    ...
    def test_interpolate(self) -> None:
        ...
        # check method that is not in pd.interpolate and
        # have not explicitly defined in TimeSeriesData code
        with self.assertRaises(ValueError):
            self.ts_univariate_missing.interpolate(
...
class TimeSeriesDataOpsTest(TimeSeriesBaseTest):
    def setUp(self) -> None:
        ...
        transformed_df_date_concat_mixed = pd.concat(
            [self.MULTIVAR_AIR_DF_DATETIME, transformed_df_date], ignore_index=True
        )
...
class TSIteratorTest(TestCase):
    def test_ts_iterator_univariate_next(self) -> None:
        ...
        ts_iterator = TSIterator(ts_data)
        val = next(ts_iterator)
        assert_series_equal(
