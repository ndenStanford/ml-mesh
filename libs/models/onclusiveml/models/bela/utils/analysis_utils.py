...
class Sample:
    ...
    def __init__(
        self, text, sample_id=None, ground_truth_entities=None, predicted_entities=None
    ):
        self.text = text
        ...
        if (
            self.ground_truth_entities is not None
            and self.predicted_entities is not None
        ):
            self.compute_scores()
    ...
    def print(self, max_display_length=1000):
        ...
        if self.ground_truth_entities is not None:
            ...
            for ground_truth_entity in self.ground_truth_entities:
                if (
                    ground_truth_entity.offset + ground_truth_entity.length
                    > max_display_length
                ):
                    continue
        ...
        if self.predicted_entities is not None:
            ...
            for predicted_entity in self.predicted_entities:
                if (
                    predicted_entity.offset + predicted_entity.length
                    > max_display_length
                ):
                    continue
...
def convert_jsonl_data_to_samples(jsonl_data: List[Dict[str, Any]]) -> List[Sample]:
    ...
    for example in jsonl_data:
        ...
        sample = Sample(
            text=example["original_text"], ground_truth_entities=ground_truth_entities
        )
        samples.append(sample)
    return samples


def convert_predictions_to_entities(
    example_predictions: Dict[str, List], text
) -> List[Entity]:
...
def convert_jsonl_data_and_predictions_to_samples(
    ...
    for sample, example_predictions in zip(samples, predictions):
        predicted_entities = convert_predictions_to_entities(
            example_predictions, sample.text
        )
        predicted_entities = [
            entity
            for entity in predicted_entities
            if entity.el_score > el_threshold and entity.md_score > md_threshold
        ]
