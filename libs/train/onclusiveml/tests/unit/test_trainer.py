...
from onclusiveml.tracking import TrackedModelSpecs, TrackedModelCard, TrackedModelTestFiles
from onclusiveml.data.feature_store import FeatureStoreParams
from onclusiveml.data.feature_store.feature_store_handle import FeatureStoreHandle
from onclusiveml.train.onclusive_model_trainer import OnclusiveModelTrainer

@pytest.fixture
def tracked_model_specs():
    return TrackedModelSpecs(project="project", model="model", api_token="api_token")  # Provide appropriate parameters
...
def tracked_model_card():
    ...
    return TrackedModelCard(model_type="trained",
        model_artifact_attribute_path="model_artifact_attribute_path",
        model_test_files=tracked_model_test_files,)  # Provide appropriate parameters

@pytest.fixture
def feature_store_params():
    return FeatureStoreParams(feast_config_bucket = "feast_config_bucket",
    config_file = "feature_store.yaml",
    local_config_dir = "local-config-dir",
    redshift_database = "redshift_database",
    redshift_schema = "feasredshift_schemat",
    redshift_table = "redshift_table",
    redshift_timestamp_field = "event_timestamp")  # Provide appropriate parameters
...
def test_onclusive_model_trainer_initialize_model(mock_boto3_client, mock_feature_store_handle, tracked_model_specs, tracked_model_card, feature_store_params):
...
@patch("onclusiveml.train.onclusive_model_trainer.FeatureStoreHandle")
@patch("onclusiveml.train.onclusive_model_trainer.boto3.client")
def test_onclusive_model_trainer_get_training_data(mock_boto3_client, mock_feature_store_handle, tracked_model_specs, tracked_model_card, feature_store_params):
    trainer = OnclusiveModelTrainer(tracked_model_specs, tracked_model_card, feature_store_params)
    
    # Mocking FeatureStoreHandle.list_entities, list_data_sources, and list_feature_views
    mock_feature_store_handle.return_value.list_entities.return_value = [MagicMock(name="entity1"), MagicMock(name="entity2")]
    mock_feature_store_handle.return_value.list_data_sources.return_value = [MagicMock(name="datasource1"), MagicMock(name="datasource2")]
    mock_feature_store_handle.return_value.list_feature_views.return_value = [MagicMock(name="feature_view1"), MagicMock(name="feature_view2")]

    # Mocking FeatureStoreHandle.fetch_historical_features
    mock_feature_store_handle.return_value.fetch_historical_features.return_value = pd.DataFrame(...)  # Provide appropriate DataFrame
...
def test_onclusive_model_trainer_upload_training_data_to_s3(mock_s3_parquet_upload, mock_boto3_client, tracked_model_specs, tracked_model_card, feature_store_params):
    trainer = OnclusiveModelTrainer(tracked_model_specs, tracked_model_card, feature_store_params)
