...
def test_compiled_feature_extraction_pipeline_from_pretrained(huggingface_pipeline_task, huggingface_model_reference, huggingface_pipeline, max_length, batch_size, neuron, sample_inputs, regression_test_atol, regression_test_rtol):
    ...
    reloaded_compiled_pipeline = CompiledPipeline.from_pretrained('test_compiled_feature_extraction_pipeline')
    
    # score re-imported compiled pipeline
    reloaded_compiled_pipeline_output: Tuple[Tuple[List[List[float]]]] = reloaded_compiled_pipeline(sample_inputs) # 1 x n_batch x n_token x n_embed
    reloaded_compiled_pipeline_output_arr: np.array = np.array(reloaded_compiled_pipeline_output)
    
    # validation: regression test embedding vectors
    np.testing.assert_allclose(reloaded_compiled_pipeline_output_arr, compiled_pipeline_output_arr, rtol=regression_test_rtol, atol=regression_test_atol)
