...
@router.get("", status_code=status.HTTP_200_OK, response_model=PromptTemplateListSchema, dependencies=[Security(get_api_key)])
def get_prompts():
    """List prompts."""
    return {
...
@router.get("/{id}", status_code=status.HTTP_200_OK, response_model=PromptTemplateSchema, dependencies=[Security(get_api_key)])
def get_prompt(id: str):
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail=f"{str(e)} - (id={str(id)})"
        )


@router.post("", status_code=status.HTTP_201_CREATED, dependencies=[Security(get_api_key)])
...
@router.put("/{id}", status_code=status.HTTP_200_OK, dependencies=[Security(get_api_key)])
...
@router.delete("/{id}", status_code=status.HTTP_200_OK, dependencies=[Security(get_api_key)])
...
@router.post("/{id}/generate", status_code=status.HTTP_200_OK, dependencies=[Security(get_api_key)])
...
# TODO: [DS-419] Add endpoints to compare prompt outputs for two prompt versions.
# TODO: [DS-420] Add analytics for prompts.
