...
var Wiky = {
  ...
  rules: {
          ...
          return (
            ...
            Wiky.store(
              "<pre" +
                ...
                Wiky.apply(
                  $3,
                  $2 ? Wiky.rules.lang[Wiky.attr($2)] : Wiky.rules.code
                ) +
                "</pre>"
            ) +
            "[p:"
      ...
      { rex: /(([\xB6])([ \t\f\v\xB6]*?)){2,}/g, tmplt: ":p]$1[p:" }, // .. separate paragraphs at blank lines ..
      {
        ...
        tmplt: function ($0, $1, $2, $3) {
          return Wiky.sectionRule(
            $1 == undefined ? "" : $1,
            "",
            Wiky.apply($2, Wiky.rules.wikiinlines),
            !$3 ? "" : $3
          );
        },
      },
    ...
    nonwikiinlines: [
      ...
      }, // inline code
      {
        ...
        tmplt: function ($0, $1) {
          return Wiky.store(
            "<code>" + Wiky.apply($2, Wiky.rules.code) + "</code>"
          );
        },
      },
    ],
  ...
  inverse: {
          ...
          return Wiky.store(
            "[" +
              ...
              Wiky.apply(
                $2,
                Wiky.hasAttr($1, "lang")
                  ? Wiky.inverse.lang[Wiky.attrVal($1, "lang").substr(2)]
                  : Wiky.inverse.code
              ) +
              "%]"
          );
    ...
    wikiblocks: [
      ...
      { rex: /\xB6{2}<li/gim, tmplt: "\xB6<li" }, // ie6 only ..
      {
        ...
        tmplt: function ($0, $1, $2) {
          return (
            $1.replace(/u/g, "*").replace(/([01aAiIg])$/, "$1.") + " " + $2
          );
        },
      }, // list items ..
      ...
      { rex: /<dt>(.*?)<\/dt>[ \f\n\r\t\v]*<dd>/gim, tmplt: "; $1: " },
      {
        ...
        tmplt: function ($0, $1) {
          return Wiky.store(
            "[" + Wiky.invStyle($1) + Wiky.invAttr($1, ["cite", "title"]) + '"'
          );
        },
      },
      ...
      { rex: /<\/table>/gim, tmplt: "]" },
      {
        rex: /<tr(?:[^>]*?)>\xB6*|<\/td>\xB6*|<tbody>\xB6*|<\/tbody>/gim,
        tmplt: "",
      },
      { rex: /<hr\/?>/gim, tmplt: "----" },
      { rex: /<br\/?>/gim, tmplt: "\\\\" },
      {
        rex: /(<p>|<(d|o|u)l[^>]*>|<\/(dl|ol|ul|p)>|<\/(li|dd)>)/gim,
        tmplt: "",
      },
  ...
  invAttr: function (str, names) {
    ...
    for (var i in names)
      if (str.indexOf(names[i] + "=") >= 0)
        a.push(
          str.replace(new RegExp("^.*?" + names[i] + '="(.*?)".*?$'), "$1")
        );
  ...
  sectionRule: function (fromLevel, style, content, toLevel) {
    ...
    for (var i = 0; i < maxlen; i++)
      if (
        fromLevel.charAt(i + 1) != toLevel.charAt(i + 1) ||
        !sync ||
        i == maxlen - 1
      ) {
    ...
    return !trf[transition]
      ...
      : trf[transition]
          ...
          .replace(
            /\$3/,
            !sectiontype ? "" : ' style="list-style-type:' + sectiontype + ';"'
          )
          .replace(/\$1/, content)
          .replace(/<p><\/p>/, "");
  },
