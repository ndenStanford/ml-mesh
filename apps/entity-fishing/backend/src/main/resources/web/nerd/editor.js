...
var editor = (function ($) {
  ...
  function updateCallTimer() {
    ...
    if (!autoCallDone) {
      ...
      if (accumulated == null || accumulated.length == 0) somethingNew = false;
      if (somethingNew) {
        createQuery();
      }
    }
  ...
  function updateAfterChange(cMirror, changeObject) {
    ...
    var nbModifiedChar =
      changeObject.text[0].length - changeObject.removed[0].length;
    //console.log('nbModifiedChar = ' + nbModifiedChar);

    // special case of the *addition* of a new line (does not appear in the change event!)
    if (nbModifiedChar == 0) {
      if (
        changeObject.from.ch == changeObject.to.ch &&
        changeObject.from.line == changeObject.to.line &&
        changeObject.origin == "+input" &&
        changeObject.text.length == 2 &&
        changeObject.text[0] == "" &&
        changeObject.text[1] == ""
      ) {
    ...
    if (nbModifiedChar == 0) {
      if (
        changeObject.from.ch != changeObject.to.ch &&
        changeObject.from.line + 1 == changeObject.to.line &&
        changeObject.origin == "+delete" &&
        changeObject.removed.length == 2 &&
        changeObject.removed[0] == "" &&
        changeObject.removed[1] == "" &&
    ...
    if (modifiedSentence == -1) modifiedSentence = sentences.length - 1;
  ...
  function createQuery() {
    ...
    query.onlyNER = onlyNER;
    (query.nbest = false), (query.processSentence = modifiedSentences);
    query.customisation = "generic";
      ...
      for (var ind = 0; ind < modifiedSentences.length; ind++) {
        if (!sentences[modifiedSentences[ind]]) {
          ...
          if (entities != null) {
            ...
            for (var m = entities.length - 1; m >= 0; m--) {
              if ($.inArray(m, toBeRemoved) != -1) query.entities.splice(m, 1);
            }
          }
        }
      }
    ...
    if (responseJson.entities) {
      ...
      for (var m = 0; m < responseJson.entities.length; m++) {
        ...
        if (indexLine < sentences.length) {
          ...
          var currentText = codeMirror.getValue();
          if (
            currentText.charAt(start) == " " &&
            currentText.charAt(end) != " " &&
          ...
          marked.push(
            codeMirror.markText(
              { line: indexLine, ch: theStart },
              { line: indexLine, ch: theEnd },
              {
                startStyle: "label " + label,
                endStyle: "",
                className: "annot-" + m,
              }
            )
  ...
  const wikimediaURL_EN =
    "https://en.wikipedia.org/w/api.php?action=query&prop=pageimages&format=json&pithumbsize=200&pageids=";
  const wikimediaURL_FR =
    "https://fr.wikipedia.org/w/api.php?action=query&prop=pageimages&format=json&pithumbsize=200&pageids=";
  const wikimediaURL_DE =
    "https://de.wikipedia.org/w/api.php?action=query&prop=pageimages&format=json&pithumbsize=200&pageids=";
  ...
  window.lookupWikiMediaImage = function (wikipedia, lang) {
    ...
    if (lang + wikipedia in imgCache) {
      ...
      var theUrl = null;
      if (lang == "fr") theUrl = wikimediaURL_FR + wikipedia;
      else if (lang == "de") theUrl = wikimediaURL_DE + wikipedia;
      else theUrl = wikimediaURL_EN + wikipedia;
  ...
  function viewEntity2() {
    ...
    if (sense) string += "<p>Sense: <b>" + sense + "</b></p>";
    string += "<p>conf: <i>" + conf + "</i></p>";
    ...
    if (freebase != null) {
      var urlImage =
        "https://usercontent.googleapis.com/freebase/v1/image" + freebase;
      urlImage += "?maxwidth=150";
      urlImage += "&maxheight=150";
      //urlImage += '&key=' + api_key;
      string += '<img src="' + urlImage + '" alt="' + freebase + '"/>';
