...
var nerd = (function ($) {
  ...
  function defineBaseURL(ext) {
    ...
    if ($(location).attr("href").indexOf("index.html") !== -1)
      baseUrl = $(location)
        .attr("href")
        .replace("index.html", "service/" + ext);
    else baseUrl = $(location).attr("href") + "service/" + ext;
    return baseUrl;
  }
  ...
  function clearQuery() {
    ...
    if (selected === "processNERDQuery" || selected === "processERDQuery") {
      const cloneQueryTemplateText = JSON.parse(
        JSON.stringify(queryTemplateText)
      );
      $("#input").attr(
        "value",
        vkbeautify.json(JSON.stringify(cloneQueryTemplateText))
      );
      $("#requestResult").html("");
    } else if (selected === "processNERDQueryPDF") {
      $("#inputFile").val("");
      const cloneQueryTemplatePDF = JSON.parse(
        JSON.stringify(queryTemplatePDF)
      );
      $("#input").attr(
        "value",
        vkbeautify.json(JSON.stringify(cloneQueryTemplatePDF))
      );
      $("#requestResult").html("");
  ...
  function submitQuery() {
    ...
    if (
      urlLocal.indexOf("language") !== -1 ||
      urlLocal.indexOf("segmentation") !== -1
    ) {
      ...
      if (
        document.getElementById("inputFile").files[0].type ===
          "application/pdf" ||
          ...
          PDFJS.getDocument(pdfAsArray).then(function (pdf) {
            ...
            for (var i = 1; i <= nbPages; i++) {
              ...
              pdf.getPage(i).then(function (page) {
                ...
                annot.setAttribute(
                  "id",
                  "detailed_annot-" + (page.pageIndex + 1)
                );
                ...
                var viewport = page.getViewport(
                  (td1.offsetWidth * 0.98) / page.getViewport(1.0).width
                );
  ...
  function displayErrorMessage(response) {
    ...
    if (response.status === 404) {
      message =
        "Not found. Concept or terms not existing or invalid identifier";
    ...
    $("#infoResult").html(
      "<font color='red'>Error encountered while requesting the server.<br/>" +
        message +
        "</font>"
    );
    responseJson = null;
  ...
  function handleSuccessfulResponse(responseText, statusText, xhr) {
    ...
    var selected = $("#selectedService").attr("value");
    if (
      selected === "processNERDQuery" &&
      responseJson.text != null &&
      responseJson.text.length > 0
    ) {
      ...
      selected === "processNERDQuery" &&
      responseJson.shortText != null &&
      responseJson.shortText.length > 0
    ) {
      SubmitSuccesfulERDSearch(responseJson, statusText);
    } else if (
      selected === "processNERDQuery" &&
      responseJson.termVector != null &&
      responseJson.termVector.length > 0
    ) {
      console.log(
        "front end for term vector disambiguation not implemented yet !"
      );
    } else if (selected === "KBTermLookup") {
  ...
  function SubmitSuccesfulNERD(responseJson, statusText) {
    ...
    if (responseJson == null || responseJson.length == 0) {
      $("#infoResult").html(
        "<font color='red'>Error encountered while receiving the server's answer: response is empty.</font>"
      );
      return;
    }
    ...
    var nbest = false;
    if (responseJson.nbest === true) nbest = true;
    ...
    if (responseJson.sentences) {
      ...
      for (var sentence in responseJson.sentences) {
        if (m == 0) {
          display +=
            '<tr class="highlight" id="sent_' + m + '" rank="' + m + '" >';
        } else {
        ...
        display +=
          '<td style="font-size:small;height:13px;color:#333;">' +
    ...
    display +=
      '<pre style="background-color:#FFF;width:95%;" id="displayAnnotatedText">';
    ...
    display +=
      '<table id="sentenceNER" style="width:100%;table-layout:fixed;" class="table">';
    ...
    if (!responseJson.sentences || responseJson.sentences.length === 0) {
      ...
      if (language) lang = language.lang;
      if (responseJson.entities) {
        ...
        for (var m = responseJson.entities.length - 1; m >= 0; m--) {
          ...
          var label = null;
          if (entity.type) label = entity.type;
          else if (domains && domains.length > 0) {
            label = domains[0].toLowerCase();
          } else label = entity.rawName;
          ...
          if (start > lastMaxIndex) {
            ...
            console.log(
              "Sorting of entities as present in the server's response not valid for this client."
            );
            ...
            string =
              ...
              "</span></span>" +
              string.substring(end, string.length + 1);
            lastMaxIndex = start;
      ...
      display +=
        '<td style="font-size:small;width:60%;border:1px solid #CCC;"><p>' +
        string +
        "</p></td>";
      display +=
        '<td style="font-size:small;width:40%;padding:0 5px; border:0"><span id="detailed_annot-0" /></td>';
      ...
      display +=
        '<td style="font-size:small;width:60%;border:1px solid #CCC;"><p><span id="sentence_ner">' +
        " " +
        "</span></p></td>";
      display +=
        '<td style="font-size:small;width:40%;padding:0 5px; border:0"><span id="detailed_annot-0" /></td>';
    ...
    display +=
      '<pre style="background-color:#FFF;width:50%;" id="displayCategories">';
    ...
    if (responseJson.global_categories) {
      ...
      display +=
        '<tr style="border-top:0px;"><td style="border-top:0px;"><span style="color:black;"><b>category</b></span></td><td style="border-top:0px;"></td><td style="border-top:0px;">';
      display +=
        '<span style="color:black;"><b>score</b></span></td><td style="border-top:0px;">';
      ...
      for (var category in categories) {
        ...
        if (score == "0.0000") continue;
        display +=
          '<tr><td><span style="color:#7F2121;">' +
          theCategory +
          "</span></td><td>" +
          '<a href="http://' +
          lang +
          ".wikipedia.org/wiki?curid=" +
          pageId +
          '" target="_blank"><img style="max-width:28px;max-height:22px;" src="resources/img/wikipedia.png"/></a></td><td>' +
          score +
          "</td>";
        display += "</tr>";
      }
    ...
    for (var sens in responseJson["entities"]) {
      ...
      if (identifier && conceptMap[identifier] == null) {
        fetchConcept(identifier, lang, function (result) {
          ...
          if (preferredTerm) $("#pref-" + localIdentifier).html(preferredTerm);
        });
      }
    }
  }
  ...
  var SubmitSuccesfulKBTermLookup = function (responseJson) {
    ...
    var lang = "en"; //default
    if (responseJson.lang) lang = responseJson.lang;
    ...
    var display =
      '<div class="note-tabs"> \
		...
		<div class="tab-content"> \
		<div>Number of ambiguous concepts: ' +
      responseJson.senses.length +
      '</div> \
		<div class="tab-pane active" id="navbar-fixed-annotation">\n';
  ...
  var SubmitSuccesfulKBConceptLookup = function (responseText, statusText) {
    ...
    display +=
      '<colgroup><col style="width: 70%;"><col style="width: 25%;"><col style="width: 5%;"></colgroup>';
    //display += '<tr style="background-color:#FFF;"><td>';
  ...
  var getPieceShowConcept = function (entity, lang) {
    ...
    var colorLabel = null;
    if (type) colorLabel = type;
    else if (domains && domains.length > 0) {
      colorLabel = domain;
    } else colorLabel = entity.preferredTerm;
    ...
    piece +=
      '<tr><td style="width:100%;border-top:0px;"><table><tr><td style="padding-left:0px;border-top:0px;"><p><b>' +
      entity.preferredTerm +
      " (" +
      lang +
      ")</b></p></td>";
    ...
    if (entity.multilingual) {
      for (var mult in entity.multilingual) {
        piece +=
          '<td style="border-top:0px;"><p>' +
          entity.multilingual[mult].term +
          " (" +
          entity.multilingual[mult].lang +
          ")</p></td>";
      }
    }
    ...
    if (definitions && definitions.length > 0)
      localHtml += wiki2html(definitions[0]["definition"], lang);
    piece +=
      '<tr><td><div class="wiky_preview_area2">' +
      localHtml +
      "</div></td></tr>";
    ...
    if (statements != null && statements.length > 0) {
      ...
      piece +=
        "<tr><td><table><tr><td style='padding:0px;border-top:0px;'><p><b>Statements: </b></p></td><td style='border-top:0px;'><div><table class='properties' style='width:100%;background-color:#fff;border:0px'>" +
        localHtml +
    ...
    piece +=
      '<span id="img-' +
      wikipedia +
      '"><script type="text/javascript">lookupWikiMediaImage("' +
      wikipedia +
      '", "' +
      lang +
      '")</script></span>';
  ...
  var getPieceShowexpandNERD = function (jsonObject, lang) {
    var piece =
      '<div class="mini-layout fluid" style="background-color:#F7EDDC;"> \
    ...
    if (jsonObject["entities"]) {
      piece +=
        '<table class="table" style="width:100%;border:1px solid white;">';
      for (var sens in jsonObject["entities"]) {
        ...
        if (type) colorLabel = type;
          ...
          colorLabel = domain;
        } else colorLabel = entity.rawName;
        ...
        if (conf && conf.length > 4) conf = conf.substring(0, 4);
        ...
        piece += "<td>";
        if (conf) piece += "<p><b>Conf</b>: " + conf + "</p>";
  ...
  var getPieceShowSenses = function (jsonObject, lang) {
    ...
    if (jsonObject["senses"]) {
      ...
      for (var sens in jsonObject["senses"]) {
        ...
        piece += "<td>";
        if (prob_c) piece += "<p><b>Cond. prob.</b>: " + prob_c + "</p>";
  ...
  const wikimediaURL_suffix =
    ".wikipedia.org/w/api.php?action=query&prop=pageimages&format=json&pithumbsize=200&pageids=";
  ...
  function viewSentenceResults(numb) {
    ...
    var currentSentence = text.substring(
      responseJson.sentences[sentence].offsetStart,
    ...
    if (responseJson.entities) {
      ...
      for (var m = responseJson.entities.length - 1; m >= 0; m--) {
        if (
          responseJson.entities[m].offsetStart >=
            responseJson.sentences[sentence].offsetStart &&
          responseJson.entities[m].offsetEnd <=
            responseJson.sentences[sentence].offsetEnd
        ) {
          var entity = responseJson.entities[m];
          var domains = entity.domains;
          ...
          //var end = parseInt(entity.offsetEnd,10);
          var start =
            parseInt(entity.offsetStart, 10) -
            responseJson.sentences[sentence].offsetStart;
          var end =
            parseInt(entity.offsetEnd, 10) -
            responseJson.sentences[sentence].offsetStart;
          ...
          if (start > lastMaxIndex) {
            ...
            currentSentence =
              currentSentence.substring(0, start) +
              '<span id="annot-' +
              m +
              '" rel="popover" data-color="' +
              label +
              ...
              label +
              '" style="cursor:hand;cursor:pointer;" >' +
              currentSentence.substring(start, end) +
              "</span></span>" +
  ...
  function annotateEntity(
    ...
    theType = "" + theType;
    if (theType) theType = theType.replace(" ", "_");
    ...
    element.setAttribute(
      "style",
      attributes + "border-width: 2px;border-style:solid; "
    ); //border-color: " + getColor(theId) +";");
    ...
    $("#annot-" + entityIndex + "-" + positionIndex).bind(
      "hover",
      viewEntityPDF
    );
    $("#annot-" + entityIndex + "-" + positionIndex).bind(
  ...
  function viewEntityPDF() {
    ...
    var localEntityNumber = parseInt(
      localID.substring(ind1 + 1, localID.length)
    );

    if (
      entityMap[localEntityNumber] == null ||
      entityMap[localEntityNumber].length == 0
    ) {
      // this should never be the case
      console.log(
        "Error for visualising annotation with id " +
          localEntityNumber +
    ...
    var string = "";
    for (
      var entityListIndex = entityMap[localEntityNumber].length - 1;
      entityListIndex >= 0;
      entityListIndex--
    ) {
      ...
      var sense = null;
      if (entity.sense) sense = entity.sense.fineSense;

      string += "<div class='info-sense-box " + colorLabel + "'";
      if (topPos != -1)
        string +=
          " style='vertical-align:top; position:relative; top:" + topPos + "'";
      ...
      string +=
        ...
        "</h3>";
      string +=
        "<div class='container-fluid' style='background-color:#F9F9F9;color:#70695C;border:padding:5px;margin-top:5px;'>" +
      ...
      if (type) string += "<p>Type: <b>" + type + "</b></p>";

      if (sense) {
        // to do: cut the sense string to avoid a string too large
        if (sense.length <= 20) string += "<p>Sense: <b>" + sense + "</b></p>";
        else {
          var ind = sense.indexOf("_");
          if (ind != -1) {
            string +=
              "<p>Sense: <b>" +
              sense.substring(0, ind + 1) +
              "<br/>" +
              sense.substring(ind + 1, sense.length) +
              "</b></p>";
          } else string += "<p>Sense: <b>" + sense + "</b></p>";
        }
      }
      if (normalized) string += "<p>Normalized: <b>" + normalized + "</b></p>";
      ...
      if (definitions != null && definitions.length > 0) {
        var localHtml = wiki2html(definitions[0]["definition"], lang);
        string +=
          "<p><div class='wiky_preview_area2'>" + localHtml + "</div></p>";
      }
      ...
      if (statements != null && statements.length > 0) {
        ...
        string +=
          "<div class='panel-heading' style='background-color:#F9F9F9;color:#70695C;border:padding:0px;font-size:small;'>";
        ...
        string +=
          "<a class='accordion-toggle collapsed' data-toggle='collapse' data-parent='#accordionParent' href='#collapseElement" +
          pageIndex +
          "' style='outline:0;'>";
        string +=
          "<h5 class='panel-title' style='font-weight:normal;'>Wikidata statements</h5>";
        string += "</a>";
        string += "</div>";
        ...
        string +=
          "<table class='statements' style='width:100%;background-color:#fff;border:1px'>" +
          localHtml +
          "</table>";
      ...
      if (valueType && valueType == "time") {
        ...
        if (statement.value) {
          ...
          if (time) {
            ...
            else localHtml += "<td>" + time.substring(1, ind) + "</td></tr>";
          }
        }
        ...
        if (statement.propertyId == "P2572") {
          ...
          if (statement.value) {
            localHtml +=
              "<td><a href='https://twitter.com/hashtag/" +
  ...
  function SubmitSuccesfulLId(responseJson, statusText) {
    ...
    if (responseJson == null || responseJson.length == 0) {
      $("#infoResult").html(
        "<font color='red'>Error encountered while receiving the server's answer: " +
    ...
    var display =
      '<pre style="background-color:#FFF;width:95%;" id="displayLanguageIdentification">';
    ...
    display +=
      "Language: <b>" + lang + "</b> with confidence of <b>" + conf + "</b>";
  ...
  function processChange() {
    ...
    if (selected === "processNERDQuery" || selected === "processERDQuery") {
      ...
      $("#input").attr(
        "value",
        vkbeautify.json(JSON.stringify(queryTemplatePDF))
      );
      $("#requestResult").html("");
    } else if (selected === "processLanguage") {
      createInputTextArea("query");
  ...
  function createSimpleTextFieldArea(nameInput, entryType) {
    ...
    var piece =
      "<table><tr>" +
      '<td><input type ="text" style="height:28px;" id="input2" name="' +
      nameInput +
      '" placeholder="Enter a ' +
      entryType +
      '..."/></td>' +
    ...
    if (entryType === "concept ID") piece += "<td></td>";
    else {
      piece +=
  ...
  function createInputTextArea(nameInput) {
    ...
    if (selected === "processNERDQueryPDF") {
      $("#field").append(
        '<table id="withExamples"><tr><td><textarea class="span7" rows="5" id="input" name="' +
          nameInput +
          '" /></td>' +
          "<td><span style='padding-left:20px;'>&nbsp;</span></td>" +
          "<td><table id='examplesBlock1'>" +
          "<tr style='line-height:130%;'><td><span id='example_pdf0' style='font-size:90%;'>" +
          examplesPDF[0] +
          "</span></td></tr>" +
          "<tr style='line-height:130%;'><td><span id='example_pdf1' style='font-size:90%;'>" +
          examplesPDF[1] +
          "</span></td></tr>" +
          "<tr style='line-height:130%;'><td><span id='example_pdf2' style='font-size:90%;'>" +
          examplesPDF[2] +
          "</span></td></tr>" +
          "<tr style='line-height:130%;'><td><span id='example_pdf3' style='font-size:90%;'>" +
          examplesPDF[3] +
          "</span></td></tr>" +
          "<tr style='line-height:130%;'><td><span id='example_pdf4' style='font-size:90%;'>" +
          examplesPDF[4] +
          "</span></td></tr>" +
          "</table></td>" +
          "</tr></table>"
      ...
      $("#field").append(
        '<table id="withExamples"><tr><td><textarea class="span7" rows="5" id="input" name="' +
          ...
          "<td><span style='padding-left:20px;'>&nbsp;</span></td>" +
          "<td><table id='examplesBlock1'>" +
          "<tr style='line-height:130%;'><td><span id='example0' style='font-size:90%;'>" +
          examples[0] +
          "</span></td></tr>" +
          "<tr style='line-height:130%;'><td><span id='example1' style='font-size:90%;'>" +
          examples[1] +
          "</span></td></tr>" +
          "<tr style='line-height:130%;'><td><span id='example2' style='font-size:90%;'>" +
          examples[2] +
          "</span></td></tr>" +
          "<tr style='line-height:130%;'><td><span id='example3' style='font-size:90%;'>" +
          examples[3] +
          "</span></td></tr>" +
          "<tr style='line-height:130%;'><td><span id='example4' style='font-size:90%;'>" +
          examples[4] +
          "</span></td></tr>" +
          "</table></td>" +
          "<td><span style='padding-left:20px;'>&nbsp;</span></td>" +
          "<td><table id='examplesBlock2'>" +
          "<tr style='line-height:130%;'><td><span id='example5' style='font-size:90%;'>" +
          examples[5] +
          "</span></td></tr>" +
          "<tr style='line-height:130%;'><td><span id='example6' style='font-size:90%;'>" +
          examples[6] +
          "</span></td></tr>" +
          "<tr style='line-height:130%;'><td><span id='example7' style='font-size:90%;'>" +
          examples[7] +
          "</span></td></tr>" +
          "<tr style='line-height:130%;'><td><span id='example8' style='font-size:90%;'>" +
          examples[8] +
          "</span></td></tr>" +
          "<tr style='line-height:130%;'><td><span id='example9' style='font-size:90%;'>" +
          examples[9] +
          "</span></td></tr>" +
          "</table></td>" +
          "<td><span style='padding-left:20px;'>&nbsp;</span></td>" +
          "<td><table id='examplesBlock3'>" +
          "<tr style='line-height:130%;'><td><span id='example10' style='font-size:90%;'>" +
          examples[10] +
          "</span></td></tr>" +
          "<tr style='line-height:130%;'><td><span id='example11' style='font-size:90%;'>" +
          examples[11] +
          "</span></td></tr>" +
          "<tr style='line-height:130%;'><td><span id='example12' style='font-size:90%;'>" +
          examples[12] +
          "</span></td></tr>" +
          "<tr style='line-height:130%;'><td><span id='example13' style='font-size:90%;'>" +
          examples[13] +
          "</span></td></tr>" +
          "<tr style='line-height:130%;'><td><span id='example14' style='font-size:90%;'>" +
          examples[14] +
          "</span></td></tr>" +
          "</table></td>" +
          "</tr></table>"
      ...
      for (index in examples) {
        $("#example" + index).bind("click", function (event) {
          ...
          else setJsonExample(examples[localIndex], false);
          $(this).removeClass("section-non-active").addClass("section-active");
        });
      }
    }
  ...
  function setJsonExample(theExample, full) {
    $.ajax({
      ...
      success: function (data) {
        ...
        else $("#input").attr("value", data["text"]);
      },
    });
  }
