...
(function () {
  ...
  Markdown.Converter = function (OPTIONS) {
    ...
    if (OPTIONS.nonAsciiLetters) {
       ...
       * */
      (function () {
        var lettersThatJavaScriptDoesNotKnowAndQ =
        ...
        asciify = function (text) {
          return text.replace(
            lettersThatJavaScriptDoesNotKnowAndQ,
            function (m) {
    ...
    function _ProcessListItems(
      ...
      list_str = list_str.replace(re, function (wholeMatch, m1, m2, m3) {
        ...
        if (contains_double_newline || last_item_had_a_double_newline) {
          ...
          item = _DoLists(
            _Outdent(item),
            /* isInsideParagraphlessListItem= */ true
          );
          item = item.replace(/\n$/, ""); // chomp(item)
    ...
    function _DoAutoLinks(text) {
      ...
      var replacer = function (wholematch, m1) {
        ...
        return (
          '<a href="' + url + '">' + pluginHooks.plainLinkText(m1) + "</a>"
        );
      };
