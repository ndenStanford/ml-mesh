...
Wiky.rules.math = {
  ...
  expr: [
    { rex: /\^\^/g, tmplt: "^&#94;" }, // ^ overindex
    {
      ...
      tmplt:
        '<span class="o"><span class="x">$3</span>$1<span class="x">$2</span></span>',
    }, // over-/underscript (\sum, \prod, \int)
    {
      rex: /(\\sum|\\prod|\\int)\^([-]?[a-zA-Z0-9\.&;#\\]+|\{@[0-9]+@\})/g,
      tmplt:
        '<span class="o"><span class="x">$2</span>$1<span>&#160;</span></span>',
    },
    {
      rex: /(\\sum|\\prod|\\int)_([-]?[a-zA-Z0-9\.&;#\\]+|\{@[0-9]+@\})/g,
      tmplt:
        '<span class="o"><span>&#160;</span>$1<span class="x">$2</span></span>',
    },
    {
      rex: /_([-]?[a-zA-Z0-9\.&;#\\]+|\{@[0-9]+@\})\^([-]?[a-zA-Z0-9\.&;#\\]+|\{@[0-9]+@\})/g,
      tmplt:
        '<span class="s"><span class="i">$2</span><span class="i">$1</span></span>',
    }, // over-/underindex
    {
      rex: /\^([-]?[a-zA-Z0-9\.&;#\\]+|\{@[0-9]+@\})/g,
      tmplt: '<sup class="i">$1</sup>',
    }, // overindex
    {
      rex: /_([-]?[a-zA-Z0-9\.&;#\\]+|\{@[0-9]+@\})/g,
      tmplt: '<sub class="i">$1</sub>',
    }, // underindex
    { rex: /-/g, tmplt: "&#8722;" },
    {
      rex: /([a-zA-Z0-9\.&;#\\]+|\{@[0-9]+@\})\/([a-zA-Z0-9\.&;#\\]+|\{@[0-9]+@\})/g,
      tmplt:
        '<span class="f"><span class="n">$1</span><span class="d">$2</span></span>',
    }, // fraction
    {
      rex: /([a-zA-Z0-9\.&;#\\]+|\{@[0-9]+@\})\/\/([a-zA-Z0-9\.&;#\\]+|\{@[0-9]+@\})/g,
...
Wiky.inverse.math = {
  ...
  expr: [
        ...
        return (
          ...
          Wiky.math
            .transpose(
              $1
                .replace(
                  /(?:^<span class=\"m\"><span class=\"e\">|<\/span><\/span>$)/g,
                  ""
                )
                .replace(/<\/span><span class=\"e\">/g, ",")
                .replace(
                  /<\/span><\/span>[^>]*<span class=\"m\"><span class=\"e\">/g,
                  "|"
                )
                .split("|")
            )
    ...
    {
      rex: /<sup>(\{?@[0-9]+@\}?)<\/sup>&#8260;<sub>(\{?@[0-9]+@\}?)<\/sub>/gim,
      tmplt: "$1//$2",
    },
...
Wiky.math = {
  toHtml: function (str) {
    var expr = function (itr) {
      ...
      var s = "";
      for (
        var c = itr.str.charAt(itr.pos++);
        itr.pos <= itr.str.length && c != "}";
    ...
    var parseTree = function (itr, endtag) {
      ...
      for (
        ...
        if (
          ...
          nam = itr.str.substring(
            itr.pos + 1,
            Math.min(idxof(itr.str, " ", itr.pos), gt)
          );
          s +=
            itr.str.substring(itr.pos, (itr.pos = gt + 1)) +
            parseTree(itr, "</" + nam + ">") +
            "</" +
            nam +
            ">";
